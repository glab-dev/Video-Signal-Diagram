...
function GenericIONode({ id, data, selected, width, height }: GenericIONodeProps) {
  ...
  // Build handles to pass as outsideHandles
  const handles = (
    <>
      {inputs.map((port) => {
  ...
  // Cascade lock toggle button (rendered outside contentRef)
  const cascadeLockToggle = showLockToggle ? (
    <button
      ...
      <table className="io-table">
        ...
        <tbody>
          {ports.map((port) => {
            ...
            const handleKey = isInput ? `input-${port.id}` : `output-${port.id}`;

            return (
              <tr
                key={port.id}
                ref={rowRef(handleKey)}
                className={`port-table-row ${handleOnOpposite ? 'handle-opposite' : ''}`}
              >
  ...
  return (
    <NodeShell
      ...
      outsideContentBefore={cascadeLockToggle}
      headerButtons={
        <button
